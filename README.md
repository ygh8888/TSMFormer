...
> on NVGesture, which we analyze quantitatively.
...
The modalities are combined only at a final **late-fusion** step (a simple average of
softmax probabilities, no learned parameters).
...
4. Reliable multi-seed measurement (after training the seeds above):
```bash
...
@article{yeo2026tsmformer,
  ...
  author  = {Yeo, Gwangho and Lee, Hyunjik and Lee, Haneum and Hwang, Leehwan},
  journal = {IEEE Access},
  year    = {2026}
}
```
