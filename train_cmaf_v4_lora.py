...
from models.lora_inject import inject_lora
...
def load_single_modal_model(cfg: dict, device) -> nn.Module:
    ...
    # B안: 약한 모달에 LoRA 주입 (backbone 본체 freeze 유지, LoRA만 학습)
    if cfg.get('lora', False):
        n_lora = inject_lora(model.backbone,
                             r=cfg.get('lora_r', 4),
                             alpha=cfg.get('lora_alpha', 8),
                             target_layer=cfg.get('lora_target', 'layer4'),
                             inject_fc=cfg.get('lora_fc', True))
        model.train()
        print(f"    -> LoRA injected: {n_lora/1e3:.1f}K trainable")
    else:
        model.eval()
...
class CMAFTrainer:
    def __init__(self, config_path: str, device):
        ...
        self.cmaf = build_cmaf_v4(
            ...
            freeze_gate=self.cfg['cmaf'].get('freeze_gate', False),
        ).to(device)

        # ── 옵티마이저: CMAF + LoRA 파라미터 (param group 분리) ──
        lora_params = []
        for model in self.modal_models:
            lora_params += [p for p in model.parameters() if p.requires_grad]
        n_cmaf = sum(p.numel() for p in self.cmaf.parameters())
        n_lora = sum(p.numel() for p in lora_params)
        base_lr = self.cfg['solver']['base_lr']
        lora_lr = self.cfg['solver'].get('lora_lr', base_lr)
        print(f"  Optimizer params: cmaf={n_cmaf/1e6:.3f}M (lr={base_lr}) + lora={n_lora/1e3:.1f}K (lr={lora_lr})")
        param_groups = [
            {'params': list(self.cmaf.parameters()), 'lr': base_lr},
    ...
    def _extract_features(self, batch_list):
        ...
        for i, (model, batch) in enumerate(zip(self.modal_models, batch_list)):
            ...
            if has_grad and self.cmaf.training:
    ...
    def train(self):
        ...
        if best_ckpt.exists():
            ...
            if 'lora_state' in ck:
                restored = 0
                for model, lsd in zip(self.modal_models, ck['lora_state']):
                    model.eval()
                    pdict = dict(model.named_parameters())
                    with torch.no_grad():
                        for n, v in lsd.items():
                            if n in pdict:
                                pdict[n].copy_(v.to(self.device))
                                restored += 1
                # 복원 검증: 첫 모달 lora_B std가 저장값과 일치하는지
                m0 = dict(self.modal_models[0].named_parameters())
                kB = next(k for k in ck['lora_state'][0] if 'lora_B' in k)
                live_std = m0[kB].float().std().item()
                save_std = ck['lora_state'][0][kB].float().std().item()
                print(f"  LoRA restored: {restored} params | live_std={live_std:.6f} save_std={save_std:.6f}")
            # eval 모드 명시적 강제 후 test
            self.cmaf.eval()
            for model in self.modal_models:
                model.eval()
